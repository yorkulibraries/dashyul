...
shinyServer(function(input, output, session) {
    ...
    output$checkouts_by_student_year_scatter_plot <- renderPlot({
        if (! input$faculty == "GS") {
            ggplot(symphony_checkouts_by_student_year %>% filter(faculty == input$faculty, subject1 == input$subject), aes(year, items)) +
            geom_boxplot() + geom_jitter(width = 0.5) +
            labs(title = paste("Symphony checkouts per student:", input$faculty, "/", input$subject, "(distinct items)"), x = "Student year", y = "") +
            theme(axis.text = element_text(size = 10), axis.text.x = element_text(angle = 45))
        } else {
            ggplot(symphony_checkouts_by_student_year %>% filter(faculty == input$faculty, subject1 == input$subject), aes(degree, items)) +
            geom_boxplot() + geom_jitter(width = 0.5) +
            labs(title = paste("Symphony checkouts per student:", input$faculty, "/", input$subject, "(distinct items)"), x = "Degree", y = "") +
            theme(axis.text = element_text(size = 10), axis.text.x = element_text(angle = 45))
        }
    })
    ...
    ##
    ## There get to be so many of these that we can't show them all.  Because most students look at very few platforms, showing only the
    ## ones with more than the median number of views is one approach to cutting off the little-used platforms.  Just using > 10
    ## ended up making for some very crowded charts in big subjects.
    ...
    output$platforms_by_student_year_scatter_plot <- renderPlot({
        if (! input$faculty == "GS") {
            ggplot(ezp_platforms_by_student_year %>% filter(faculty == input$faculty, subject1 == input$subject), aes(year, platforms)) +
            geom_boxplot() + geom_jitter(width = 0.5) +
            labs(title = paste("EZProxy platforms per student:", input$faculty, "/", input$subject), x = "Student year", y = "") +
            theme(axis.text = element_text(size = 10), axis.text.x = element_text(angle = 45))
        } else {
            ggplot(ezp_platforms_by_student_year %>% filter(faculty == input$faculty, subject1 == input$subject), aes(degree, platforms)) +
            geom_boxplot() + geom_jitter(width = 0.5) +
            labs(title = paste("EZProxy platforms per student:", input$faculty, "/", input$subject), x = "Degree", y = "") +
            theme(axis.text = element_text(size = 10), axis.text.x = element_text(angle = 45))
        }
    })

    output$sp_ebook_most_viewed_table <- renderTable({
        sp_ebook_most_viewed %>% filter(faculty == input$faculty, subject1 == input$subject) %>% arrange(desc(viewed)) %>% head(10) %>% left_join(sp_ebook_id_map, by = c("ebook_id" = "complete_id")) %>% mutate(record_link = link_to_sp_books(ebook_id, title)) %>% select(viewed, record_link)
    }, include.rownames = FALSE, sanitize.text.function = function(x) x)
