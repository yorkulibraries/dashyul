...
## yudesk_data_dir <- paste0(Sys.getenv("DASHYUL_DATA"), "/yudesk/")
yudesk_data_dir <- "/dashyul/data/yudesk/"

yudesk_data_file <- paste0(yudesk_data_dir, "yudesk-summary.csv.gz")

l <- read_csv(yudesk_data_file)
...
checkboxGroupInput("ayear", "Academic year", 2010:2017, selected = 2010:2017)
...
construct_annual_chart <- function(library_name = NULL, location_name = NULL) {
    ...
    annual_data <- annual_data %>% group_by(ayear) %>% summarise(total = sum(count))
    if (! is.null(library_name)) {
...
construct_monthly_chart <- function(library_name = NULL, location_name = NULL) {
    ...
    monthly_data <- monthly_data %>% group_by(ayear, month_name) %>% summarise(total = sum(count))
    if (! is.null(library_name)) {
        chart_title <- library_name
    } else {
        chart_title <- "YUL"
