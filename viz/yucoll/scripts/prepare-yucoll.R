...
## We'll look at things from before this year, because
## it is not yet complete (unless it's the end of August,
## in that case, just wait for September).
...
## Trim down all the holdings to just things that can circulate.
## There's no point dealing with all of the periodicals and microfiche
## and such.

items <- read_csv(catalogue_current_item_details_file, col_types = "") %>%
    filter(class_scheme == "LC") %>%
    mutate(acq_ayear = academic_year(acq_date)) %>%
    filter(acq_ayear >= 2000, acq_ayear < current_academic_year) %>%
    select(item_barcode, control_number, lc_letters, lc_digits, home_location, item_type, acq_ayear) %>%
    filter(home_location %in% c("SCOTT", "SCOTT-LEIS", "STEACIE", "FROST", "BRONFMAN", "SCOTT-MAPS", "LAW")) %>%
    filter(item_type %in% c("SCOTT-BOOK", "STEAC-BOOK", "FROST-BOOK", "BRONF-BOOK", "SCORE", "MAP", "SCMAP-BOOK", "LAW-BOOK"))
...
## Join items with checkouts: the left join means that all the items
## are there, and any checkout information is added. There will be
## duplicate rows, because some items have been checked out multiple
## times, but we don't care how many times they've been checked out,
## just whether or not they have. We want things broken by by location
## and item_type, and within that lc_letters and lc_digits, so group
## by those and acq_ayear, and for each of those count up the number
## of items that have ever circed and those that never have.

yucoll_data <- left_join(items, checkouts) %>%
    mutate(has_circed = ! is.na(circ_ayear)) %>%
    distinct(item_barcode, lc_letters, lc_digits, acq_ayear, home_location, item_type, has_circed) %>%
    group_by(lc_letters, lc_digits, acq_ayear, home_location, item_type) %>%
    mutate(total = n(),
           circed = sum(has_circed),
           uncirced = total - circed) %>%
    ungroup() %>%
    select(acq_ayear, lc_letters, lc_digits, home_location, item_type, circed, uncirced) %>%
    distinct()
